...
import { ValidationErrorInterface } from '@utils/types/validation-error.interface';
...
export class I18nExceptionFilterPipe implements ExceptionFilter {
  ...
  consoleLog(exception: any) {
    ...
    if (exception.response.errors) {
      message = JSON.stringify(exception.response.errors);
    }
